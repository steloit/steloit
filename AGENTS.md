...
6. **Dual auth context keys** — SDK routes (`/v1/*`) set `SDKAuthContextKey`, `APIKeyIDKey`, `ProjectIDKey`. Dashboard routes (`/api/v1/*`) set `AuthContextKey`, `UserIDKey`. Using `MustGetUserID()` in an SDK handler or `GetSDKAuthContext()` in a dashboard handler silently returns zero values (or panics). Always match the getter to the route group.
13. **User ID helpers** — Handlers on `RequireAuth`-protected routes use `userID := middleware.MustGetUserID(c)` (idiomatic Go `Must*` convention; panic caught by Recovery → 500 on misconfiguration). Handlers on `OptionalAuth` routes use `userID, ok := middleware.GetUserIDFromContext(c)` and branch on `ok`. Never defensively combine both.
